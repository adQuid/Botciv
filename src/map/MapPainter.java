...
public class MapPainter implements Runnable{
	...
	public void paintImage(BufferedImage img, int xCoord, int yCoord) {
		...
		BufferedImage scaledImage = ImageUtilities.scale(img,(int)Math.ceil(MainUIMapDisplay.getMapHeight()/(1.0*MainUI.visionDistance)),(int)Math.ceil(MainUIMapDisplay.getMapHeight()/(1.0*MainUI.visionDistance)));
			...
			for(int y = coord.y; y < coord.y + (MainUIMapDisplay.getMapHeight()/MainUI.visionDistance); y++) {
				if(x > 0 && y > 0 && x < MainUIMapDisplay.getMapWidth() && y < MainUIMapDisplay.getMapHeight()) {
					try {
						...
						if(alpha > 0) {
							MainUIMapDisplay.map.setRGB(x, y, scaledImage.getRGB(x - coord.x, y - coord.y));
						}
