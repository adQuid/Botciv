...
public class GameUnitTests {
	...
	public void testImage() {
		...
		BotcivGame imageGame = (BotcivGame)testGame.imageForPlayer(player1);
		BotcivGame copyGame = (BotcivGame)(new BotcivGameCloner()).cloneGame(testGame);
		
		assert(imageGame.world.getTileAt(new Coordinate(5,5)).getAllUnits().size() == 1);
		assert(imageGame.world.getTileAt(new Coordinate(5,6)).getAllUnits().size() == 1);
		assert(imageGame.getUnits().size() == 2);
